...
- **La vacante dice dónde está publicada**: la ficha muestra la **URL del aviso en texto copiable** (no solo el botón) con **Copiar URL**, y el listado tiene **Aplicar ↗** en cada fila para postular sin abrir la ficha. Cuando la fuente es un agregador se muestra el **portal de origen** que ese agregador informa (Jooble devuelve `source: "fitly.work"` o `"grabjobs.co"`). Sin eso, un buen match en un agregador es una vacante imposible de encontrar.
